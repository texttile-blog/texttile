defmodule TexttileWeb.LinkHTML do
  ...
  def show(assigns) do
    ...
    <Layouts.auth subtitle={
      ...
      >
        <div class="mb-[7px]">
          <label class="lab block mb-0" for="link-password">New password</label>
          <input
            type="password"
            id="link-password"
            name="user[password]"
      ...
      <p class="note mt-[22px] leading-[1.6]">
        ...
        password, and nobody else can read it.
      </p>
      <p class="mt-[13px]"><a class="link text-[13px]" href={~p"/login"}>Back to sign-in</a></p>
    </Layouts.auth>
    """
  end
  ...
  def dead(assigns) do
    ...
    <Layouts.auth subtitle={"A new password · #{@conn.host}"}>
      ...
      <p class="text-[13.5px] mt-[9px] leading-[1.6]">
        A link to set a password works one time, and for 24 hours. Somebody
        used this one already, or it is older than a day.
      </p>
      <p class="note mt-[10px] leading-[1.6]">
        Ask for a new one and it arrives in the same inbox. The old link stays
        dead.
      </p>
      <p class="flex gap-2 mt-[20px]">
        <a class="btn solid" href={~p"/forgot"}>Request a new link</a>
        <a class="btn quiet" href={~p"/login"}>Back to sign-in</a>
      </p>
    </Layouts.auth>
    """
  end
    ...
    <Layouts.auth subtitle={"A new password · #{@conn.host}"}>
      <%= if @sent do %>
        <p class="text-[13.5px] mt-[20px] leading-[1.6]" id="forgot-sent">
          The mail is on its way, if an account uses that address. Open it and
          follow the link.
        </p>
        <p class="note mt-[10px] leading-[1.6]">
          This screen never says whether an address has an account. The link
          works one time, and for 24 hours. No mail arrives for an address
          without an account.
        </p>
      <% else %>
        <p class="text-[13.5px] mt-[20px] leading-[1.6]">
          Type the email address of your account. A link to set a new password
        ...
        >
          <div class="mb-[15px]">
            <label class="lab block mb-0" for="forgot-email">Email address</label>
            <input type="email" id="forgot-email" name="user[email]" autocomplete="email" autofocus />
          </div>
          <button class="btn solid w-full h-[38px] mt-[6px]" type="submit">Send the link</button>
        </.form>
      <% end %>
      <p class="mt-[20px]"><a class="link text-[13px]" href={~p"/login"}>Back to sign-in</a></p>
    </Layouts.auth>
    """
  end
end
