...
# Where the SQLite database and the uploaded files live. Both paths can be
# overridden by the host; point them at persistent volumes.
ENV DATABASE_PATH="/data/texttile.db"
ENV UPLOADS_PATH="/data/uploads"

# Only copy the final release from the build stage
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/texttile ./
COPY --chmod=755 docker-entrypoint.sh /app/docker-entrypoint.sh

# The entrypoint prepares the data directories as root, then drops to nobody.
